...
public class SecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.authorizeHttpRequests(auth -> auth.dispatcherTypeMatchers(DispatcherType.FORWARD).permitAll()
				...
				.requestMatchers("/mamager/**").hasRole("Manager") // 매니저기능: 인가()
				.requestMatchers("/admin/**").hasRole("ADMIN") // 매니저기능: 인가()
				...
				.anyRequest().permitAll() // 그 외 모든 요청은 인증, 인가가 필요 없음.
		);
		...
		/*
		 * // 4.기본폼 로그인을 활성화 //httpSecurity.formLogin(Customizer.withDefaults());
